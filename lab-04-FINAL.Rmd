...
text ├── plot.title ├── plot.subtitle ├── axis.title ├── axis.title.x / axis.title.y └── legend.text / legend.title
...
df_line <- apt_rent_rel %>%
  mutate(month = as.Date(month)) %>%
  filter(!is.na(month), !is.na(rent_change)) %>%
  group_by(month) %>%
  summarise(rent_index = mean(1 + rent_change, na.rm = TRUE), .groups = "drop") %>%
  arrange(month)

ggplot(df_line, aes(month, rent_index)) +
  geom_hline(yintercept = 1, linetype = "dashed", linewidth = 0.5, alpha = 0.7) +
  geom_line(linewidth = 1.15, color = "#1f77b4") +
  geom_point(size = 1.4, color = "#1f77b4") +
  scale_y_continuous(labels = percent_format(scale = 100),
                     name = "Rent Index (Dec 2019 = 100)") +
  labs(title = "WA Rent Index vs Dec 2019", x = NULL) +
...
ggplot(df_line, aes(month, rent_index)) +
  geom_area(fill = "#2E8B57", alpha = 0.9) +
  scale_y_reverse(labels = percent_format(scale = 100),
                  name = "Affordability Index (Dec 2019 = 100)") +
  labs(title = "Affordability Trend Over Time", x = NULL) +
  theme_minimal(base_size = 12) +
  theme(panel.grid.minor = element_blank(),
        axis.text.x = element_text(angle = 45, hjust = 1))
```

The misleading version of the visualization distorts the viewer’s interpretation by reversing the y-axis, causing higher rent values to appear lower on the chart. This inversion creates the illusion that housing affordability has improved when, in reality, rents have increased over time. The use of a filled green area further reinforces this false impression—green is typically associated with positive outcomes, prompting viewers to subconsciously read the downward slope as “good.” Together, the inverted scale and deceptive color cue make the chart appear optimistic at first glance, even though it conveys the opposite of the true trend.
