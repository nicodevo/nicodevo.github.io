...
sales_join <- kingco_sales %>%
  ...
  mutate(
    ...
    total_val = coalesce(land_val.x, land_val.y) + coalesce(imp_val.x, imp_val.y),
    ppsf = price / pmax(coalesce(sqft.x, sqft.y), 1),
    year = year(sale_date),
    region = if_else(city %in% eastside, "Eastside", "Other")
  )
...
This analysis explores home sales in King County’s Eastside cities since 2020. After joining and cleaning the data, I calculated key metrics like median sale price, average price per square foot, and median total property value by city. The results show that Bellevue has the highest overall values, with noticeable variation across the Eastside, highlighting market differences within the region.
