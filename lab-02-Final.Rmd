...
Interesting, there were only 10 transactions of properties with views of both Lake Washington and Lake Samammish after the onset of the pandemic (2020). Let's look at how many of them are below 1.5 millions.
...
kingco_sales_add <- kingco_sales %>% 
  mutate(
    ...
    view_lakes = ifelse(view_lakewash == 1 & view_lakesamm == 1, 1, 0)
  )
```
...
kingco_sales_add <- kingco_sales_add %>% 
  rename(view_lakewash_lakesamm = view_lakes)
```
...
# 1
kingco_sel <- kingco_homes %>%
  select(pinx, city, land_val, imp_val, sqft, view_lakewash, view_lakesamm)

# 2
kingco_bellevue <- kingco_sel %>%
  mutate(city = toupper(city)) %>%
  filter(city == "BELLEVUE")

# 3
kingco_bellevue <- kingco_bellevue %>%
  mutate(total_val = land_val + imp_val)

# 4
kingco_bellevue_top10 <- kingco_bellevue %>%
  arrange(desc(total_val)) %>%
  head(10)

kingco_bellevue_top10

# 5
bellevue_summary <- kingco_bellevue %>%
  summarize(
...
seattle_summary <- kingco_homes_clean %>%
  ...
  arrange(desc(avg_total_val_per_sqft))  
...
sample_means_n10 <- replicate(
  reps_small,
  bellevue_after2020 %>%
    sample_n(n_small, replace = TRUE) %>%
...
se_n10_empirical <- sd(sample_means_n10, na.rm = TRUE)
se_n10_theory <- sd(bellevue_after2020$sale_price, na.rm = TRUE) / sqrt(n_small)

se_n10_empirical
se_n10_theory

summary(sample_means_n10)
```

I found that even with a small sample size of 10 and only 200 repetitions, the distribution of sample means still clustered around the true population mean, though with greater variability. The simulation shows that as we take repeated random samples, the distribution of their means approaches a normal distribution centered around the population mean, illustrating the central limit theorem even when the original data is skewed.
...
eastside <- c("BELLEVUE","REDMOND","KIRKLAND")

sales_join <- kingco_sales %>%
  left_join(kingco_homes_clean, by = "pinx") %>%
  mutate(
    price = sale_price,
    city = toupper(coalesce(city.x, city.y)),
    land_val = coalesce(land_val.x, land_val.y),
    imp_val = coalesce(imp_val.x, imp_val.y),
    sqft = coalesce(sqft.x, sqft.y),
    beds = coalesce(beds.x, beds.y),
    bath_full = coalesce(bath_full.x, bath_full.y),
    bath_3qtr = coalesce(bath_3qtr.x, bath_3qtr.y),
    bath_half = coalesce(bath_half.x, bath_half.y),
    lot_size = coalesce(sqft_lot.x, sqft_lot.y),
    view_lakewash = coalesce(view_lakewash.x, view_lakewash.y),
    view_lakesamm = coalesce(view_lakesamm.x, view_lakesamm.y)
  ) %>%
  mutate(
    total_val = land_val + imp_val,
    ppsf = price / pmax(sqft, 1),
    year = year(sale_date),
    region = case_when(city %in% eastside ~ "Eastside", TRUE ~ "Other")
  ) %>%
  distinct(pinx, sale_date, .keep_all = TRUE) %>%
  select(sale_id, pinx, sale_date, year, city, region, price, sqft, ppsf, beds,
         bath_full, bath_3qtr, bath_half, lot_size, land_val, imp_val, total_val,
         view_lakewash, view_lakesamm)

eastside_summary <- sales_join %>%
  filter(year >= 2020, region == "Eastside") %>%
  group_by(city) %>%
  summarise(
    n = n(),
    median_price = median(price, na.rm = TRUE),
    mean_ppsf = mean(ppsf, na.rm = TRUE),
    med_total_val = median(total_val, na.rm = TRUE),
    .groups = "drop"
  ) %>%
...
top5 <- sales_join %>%
  filter(year >= 2020, region == "Eastside") %>%
  group_by(city) %>%
  slice_max(order_by = price, n = 5, with_ties = FALSE) %>%
  ungroup() %>%
  select(pinx, city, year, price, sqft, ppsf, beds, bath_full, bath_3qtr, bath_half, lot_size) %>%
  arrange(desc(price))

sample_view <- sales_join %>%
  filter(year >= 2020, region == "Eastside") %>%
  sample_n(20) %>%
  select(pinx, city, year, price, sqft, ppsf, view_lakewash, view_lakesamm)

eastside_summary
top5
sample_view
summary(sales_join$price)
```

From this exploration, we can see that Bellevue consistently has the highest median sale prices and property values per square foot among Eastside cities since 2020. The sample and top sales data show a wide range of property prices, and the summary statistics confirm a right-skewed distribution influenced by a small number of very high-value homes.
