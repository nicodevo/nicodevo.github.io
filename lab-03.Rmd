...
hvi_tidy_filled <- hvi_tidy %>%
  arrange(RegionName, Date) %>%
  group_by(RegionName) %>%
  mutate(HVI = zoo::na.approx(HVI, x = Date, na.rm = FALSE, rule = 2)) %>%
  ungroup()

sum(is.na(hvi_tidy$HVI))
sum(is.na(hvi_tidy_filled$HVI))
```
...
nmbd <- readr::read_csv("nmdb-outstanding-mortgage-statistics-states-quarterly.csv", show_col_types = FALSE)
...
yr_intrate <- nmbd %>%
  filter(SERIESID == "AVE_INTRATE", !is.na(VALUE1)) %>%
  summarise(yr = max(YEAR, na.rm = TRUE)) %>% pull(yr)

q_intrate <- nmbd %>%
  filter(SERIESID == "AVE_INTRATE", YEAR == yr_intrate, !is.na(VALUE1)) %>%
  summarise(q = max(QUARTER, na.rm = TRUE)) %>% pull(q)

stats_intrate <- nmbd %>%
  filter(SERIESID == "AVE_INTRATE", YEAR == yr_intrate, QUARTER == q_intrate) %>%
  mutate(group = if_else(GEONAME == "Washington", "Washington", "Rest of US")) %>%
  group_by(group) %>%
  summarise(
    mean   = mean(VALUE1, na.rm = TRUE),
    median = median(VALUE1, na.rm = TRUE),
    min    = suppressWarnings(min(VALUE1, na.rm = TRUE)),
    p25    = suppressWarnings(quantile(VALUE1, 0.25, na.rm = TRUE)),
    p75    = suppressWarnings(quantile(VALUE1, 0.75, na.rm = TRUE)),
    max    = suppressWarnings(max(VALUE1, na.rm = TRUE)),
    .groups = "drop"
  ) %>%
  mutate(across(c(min, p25, p75, max), ~ ifelse(is.infinite(.x), NA_real_, .x)))

nmbd_wide <- nmbd %>%
  select(GEONAME, GEOID, YEAR, QUARTER, SERIESID, VALUE1) %>%
  pivot_wider(names_from = SERIESID, values_from = VALUE1)

n_after <- nrow(nmbd_wide)
p_after <- ncol(nmbd_wide)

rows_cols_report <- tibble(rows_before = n_before, cols_before = p_before,
                           rows_after = n_after,  cols_after  = p_after)

ts_intrate <- nmbd %>%
  filter(SERIESID == "AVE_INTRATE") %>%
  mutate(group = if_else(GEONAME == "Washington", "Washington", "Rest of US"),
         t = YEAR + (QUARTER - 1) / 4) %>%
  group_by(group, t) %>%
  summarise(value = mean(VALUE1, na.rm = TRUE), .groups = "drop")

p_ts <- ggplot(ts_intrate, aes(x = t, y = value, color = group)) +
  geom_line() +
  geom_point(size = 0.9) +
  labs(title = "Average Mortgage Interest Rate — WA vs Rest of US",
       x = "Year", y = "Average Interest Rate") +
  theme_minimal()

yr_ltv <- nmbd %>%
  filter(SERIESID == "PCT_MTMLTV_GT100", !is.na(VALUE1)) %>%
  summarise(yr = max(YEAR, na.rm = TRUE)) %>% pull(yr)

q_ltv <- nmbd %>%
  filter(SERIESID == "PCT_MTMLTV_GT100", YEAR == yr_ltv, !is.na(VALUE1)) %>%
  summarise(q = max(QUARTER, na.rm = TRUE)) %>% pull(q)

mtmltv_gt100 <- nmbd %>%
  filter(SERIESID == "PCT_MTMLTV_GT100", YEAR == yr_ltv, QUARTER == q_ltv) %>%
  mutate(group = if_else(GEONAME == "Washington", "Washington", "Rest of US")) %>%
  group_by(group) %>%
  summarise(value = mean(VALUE1, na.rm = TRUE), .groups = "drop")

p_ltv <- ggplot(mtmltv_gt100, aes(x = group, y = value, fill = group)) +
  geom_col(width = 0.6, show.legend = FALSE) +
  labs(title = paste0("Share of Loans with MTM LTV > 100% (Q", q_ltv, " ", yr_ltv, ")"),
       x = NULL, y = "Percent") +
  theme_minimal()

yr_pay <- nmbd %>%
  filter(SERIESID == "AVE_PAYMENT", !is.na(VALUE1)) %>%
  summarise(yr = max(YEAR, na.rm = TRUE)) %>% pull(yr)

q_pay <- nmbd %>%
  filter(SERIESID == "AVE_PAYMENT", YEAR == yr_pay, !is.na(VALUE1)) %>%
  summarise(q = max(QUARTER, na.rm = TRUE)) %>% pull(q)

pay_box <- nmbd %>%
  filter(SERIESID == "AVE_PAYMENT", YEAR == yr_pay, QUARTER == q_pay)

p_box <- ggplot(pay_box, aes(x = reorder(GEONAME, VALUE1, na.rm = TRUE), y = VALUE1)) +
  geom_boxplot() +
  coord_flip() +
  labs(title = paste0("Distribution of Average Mortgage Payment by State (Q", q_pay, " ", yr_pay, ")"),
       x = NULL, y = "Average Payment") +
  theme_minimal()

yr_scatter <- min(yr_intrate, yr_pay, na.rm = TRUE)
q_scatter <- min(q_intrate, q_pay, na.rm = TRUE)

intrate_scatter <- nmbd %>%
  filter(YEAR == yr_scatter, QUARTER == q_scatter,
         SERIESID %in% c("AVE_INTRATE", "AVE_PAYMENT")) %>%
  select(GEONAME, SERIESID, VALUE1) %>%
  pivot_wider(names_from = SERIESID, values_from = VALUE1) %>%
  filter(!is.na(AVE_INTRATE), !is.na(AVE_PAYMENT))
...
The NMDB data shows that Washington’s mortgage market trends closely follow the rest of the U.S., with average interest rates moving in parallel over time and staying near the national average. The share of loans with mark-to-market LTV above 100% is also similar to other states, suggesting comparable risk levels. The distribution of average mortgage payments highlights variation across states, but Washington remains near the midpoint, indicating a balanced housing finance environment. Overall, these patterns suggest Washington’s mortgage market behaves consistently with broader national trends.
